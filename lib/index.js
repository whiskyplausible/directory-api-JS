...
(function() {
	...
	this.directory = function(resolve, reject)
	{
		...
		.then(function(json)
		{
			...
			if (metadata && metadata.jsonPathTranslations)
			{
				JSPON.setSettings({refReplacementsOneToOne: metadata.jsonPathTranslations});
			}

			//console.log(JSPON);

			directoryData.data = JSPON.jsponParse({}, json, JSPON.jsonPathRoot);
